...
class FiveSectionChannelConfig:
    ...
    def get_five_section_geometry(self) -> List[Dict]:
        ...
        for i, section_data in enumerate(self.sections_data):
            section_config = {
                ...
                'bottom_width': section_data.bottom_width,
                'side_slope': section_data.side_slope,
                'roughness': section_data.roughness,
                'shape': section_data.shape,
                
                # 水力要素计算函数
                'area_func': self._create_area_function(section_data),
                'top_width_func': self._create_top_width_function(section_data),
                'hydraulic_radius_func': self._create_hydraulic_radius_function(section_data),
                'conveyance_func': self._create_conveyance_function(section_data),
                
                # 几何验证函数
                'is_valid_water_level': self._create_water_level_validator(section_data),
                'get_normal_depth': self._create_normal_depth_calculator(section_data)
            }
    ...
    def _create_area_function(self, section: CrossSectionGeometry) -> Callable:
        """
        创建过水面积计算函数
        ...
        def area_function(H: float) -> float:
            if H <= Z:
                return 0.0
            depth = H - Z
            return B * depth + m * depth * depth
        
        return area_function
    
    def _create_top_width_function(self, section: CrossSectionGeometry) -> Callable:
        """
        创建水面宽度计算函数
        ...
        Returns:
        --------
        Callable: 水面宽度计算函数 T(H)
        """
        B = section.bottom_width
        Z = section.elevation
        m = section.side_slope
        
        def top_width_function(H: float) -> float:
            if H <= Z:
                return 0.0
            depth = H - Z
            return B + 2 * m * depth
        
        return top_width_function
    
    def _create_hydraulic_radius_function(self, section: CrossSectionGeometry) -> Callable:
        """
        创建水力半径计算函数
        ...
        Returns:
        --------
        Callable: 水力半径计算函数 R(H)
        """
        B = section.bottom_width
        Z = section.elevation
        m = section.side_slope
        area_func = self._create_area_function(section)
        
        def hydraulic_radius_function(H: float) -> float:
            if H <= Z:
                return 0.0
            ...
            # 湿周 = 底宽 + 2×边坡长度
            perimeter = B + 2 * depth * math.sqrt(1 + m * m)
        ...
        return hydraulic_radius_function
    
    def _create_conveyance_function(self, section: CrossSectionGeometry) -> Callable:
        """
        创建输水能力计算函数
        ...
        Returns:
        --------
        Callable: 输水能力计算函数 K(H)
        """
        n = section.roughness
        area_func = self._create_area_function(section)
        hydraulic_radius_func = self._create_hydraulic_radius_function(section)
        
        def conveyance_function(H: float) -> float:
            if H <= section.elevation:
                return 0.0
            
            area = area_func(H)
            hydraulic_radius = hydraulic_radius_func(H)
            
            if area == 0 or hydraulic_radius == 0:
                return 0.0
            
            return (1.0 / n) * area * (hydraulic_radius ** (2.0/3.0))
        
        return conveyance_function
    
    def _create_water_level_validator(self, section: CrossSectionGeometry) -> Callable:
        """
        创建水位有效性验证函数
        ...
        def water_level_validator(H: float) -> bool:
            """验证水位是否有效"""
            return H > Z and H < Z + 10.0  # 限制最大水深为10m
        
        return water_level_validator
    
    def _create_normal_depth_calculator(self, section: CrossSectionGeometry) -> Callable:
        """
        创建正常水深计算函数（恒定均匀流）
        
        使用Manning公式：Q = K × S^0.5
        其中 S 为河床坡度
