...
WaterNet的参数估计功能通过`ParameterEstimator`类，实现了从高精度物理模型到降阶模型的高效、准确的参数传递。该功能涵盖了静态特征曲线辨识和动态参数优化两大核心能力，为构建可靠、高效的水力仿真系统提供了坚实的基础。通过提供的示例和灵活的API，用户可以轻松地将此功能集成到自己的工作流中。